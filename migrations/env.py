...
from products.settings import __logging_settings
...
def configure_alembic_logging() -> None:
    """Настройка логирования для Alembic и SQLAlchemy с использованием structlog"""
    formatter = structlog.stdlib.ProcessorFormatter(
        processor=__logging_settings.processors[-1],
        foreign_pre_chain=__logging_settings.processors[:-1],
    )
    ...
    for name in ["alembic", "sqlalchemy.engine"]:
        ...
        alembic_logger.handlers = [handler]
...
async def run_migrations_online() -> None:
    ...
    if connectable is None:  # pyright: ignore[reportUnnecessaryComparison]
        msg = (
            ...
            "Please ensure your `pyproject.toml` according to the official Alembic documentation."
        )
        raise RuntimeError(
            msg,
        )
